...
exports.details = function (req, res) {
    ...
    Session.findOne({ _id: session_id }, function(err, session) {
        ...
        Place.findOne({ name: req.body.name }, function(err, item) {
            if (err || item === null) {
                console.log("Place " + req.body.name + " not found.");
                res.json({status: false, err: err});
                return console.error(err);
            }

            res.json({status: true, details: item});
            console.log("Place details" + item);
        });
        return;
    });
};
...
exports.create = function (req, res) {
    ...
    Place.findOne({ name: req.body.name }, function(err, route) {
        if (err || route !== null) {
            console.log("Place " + req.body.name + " already created.");
            if (route !== null) {
                res.json({status: false, err: "Place " + req.body.name + " already created."});
            } else {
        ...
        var item = new Place({name: req.body.name, owner: req.body.owner});
        ...
        console.log("New Place details:" + item);
    });
};
